...
func startWebsocketHandler(host string) {
	...
	for {
		...
		default:
			...
			connectionBroken := handleConnection(c, interrupt)
...
func handleConnection(c *websocket.Conn, interrupt chan os.Signal) bool {
		...
		for {
			...
			if messageType == websocket.TextMessage {
				if meterReading := types.MeterReadingFromJsonBytes(message); meterReading != nil {
					handleMeterReading(meterReading)
				} else {
